...
def apply_soc_name(data):
    ...
    data.loc[data['SOC_NAME'].isnull(), 'SOC_NAME'] = data.apply(
        lambda x: match_soc_name(x['EMPLOYER_NAME'], x['JOB_TITLE'], x['FULL_TIME_POSITION']) if pd.isnull(
            x['SOC_NAME']) else "", axis=1)
    return data
...
if __name__ == '__main__':
    ...
    now = time.gmtime(end_time - start_time)
